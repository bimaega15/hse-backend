...
// =====================================================================
// One-time helper: buat "storage:link" lewat browser.
// Untuk shared hosting yang memblokir `php artisan storage:link`
// (fungsi symlink() / exec() dimatikan di CLI).
//
// Cara pakai: buka  https://baiktech.com/setup-storage?token=baiktech-2026
// Coba symlink() dulu (sering aktif di web/PHP-FPM walau mati di CLI);
// kalau diblokir, otomatis fallback menyalin file ke public/storage.
//
// ⚠️ GANTI token di bawah, dan HAPUS route ini setelah gambar muncul.
// =====================================================================
Route::get('/setup-storage', function (\Illuminate\Http\Request $request) {
    $secret = 'baiktech-2026'; // ⚠️ ganti & rahasiakan

    abort_unless($request->query('token') === $secret, 403, 'Invalid token');
    ...
    $log = [
        'target'                    => $target,
        'link'                      => $link,
        'symlink_function_available'=> function_exists('symlink'),
    ];

    // Bersihkan symlink yang rusak (link ada tapi target hilang)
    if (is_link($link) && ! file_exists(readlink($link))) {
        @unlink($link);
        $log['removed_broken_symlink'] = true;
    }
    ...
    if (file_exists($link)) {
        return response()->json([
            ...
            'message' => 'public/storage sudah ada — tidak ada yang diubah. Coba akses gambar langsung.',
            'log'     => $log,
        ]);
    }

    // Metode 1: symlink asli (paling ideal — upload baru langsung tampil)
    if (function_exists('symlink') && @symlink($target, $link)) {
        return response()->json([
            'status'  => 'success',
            'method'  => 'symlink',
            'message' => 'Symlink berhasil dibuat. Gambar sekarang harusnya muncul. HAPUS route ini.',
            'log'     => $log,
        ]);
    }
    $log['symlink_attempt'] = 'gagal / diblokir host';

    // Metode 2: fallback — salin folder jadi direktori nyata
    $copy = function (string $src, string $dst) use (&$copy): int {
        if (! is_dir($src)) {
            return 0;
        }
        if (! is_dir($dst)) {
        ...
        foreach (scandir($src) as $item) {
            ...
            $count += is_dir($s) ? $copy($s, $d) : (int) @copy($s, $d);
        }
        return $count;
    };
    ...
    return response()->json([
        ...
        'method'       => 'copy',
        'files_copied' => $copied,
        'message'      => "Symlink diblokir host. {$copied} file disalin ke public/storage. "
            . 'PENTING: upload BARU tidak otomatis muncul — akses ulang route ini setelah ada upload, '
            . 'atau minta solusi disk permanen (tanpa symlink).',
        'log'          => $log,
    ]);
});
